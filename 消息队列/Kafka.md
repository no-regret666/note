...
Kafka集群持久化保存（使用可配置的保留期限）所有发布记录——无论它们是否被消费。数据量达到上限（比如1G）或者数据达到过期时间（比如7天），Kafka就会删除旧消息。

kafka对消息的存储和缓存严重依赖于文件系统。
...
kafka 中的消息就是 topic，topic 只是逻辑上的概念，而 partition 才是物理上概念。Log 是 Kafka 用于表示日志文件的组件。每个 Partiton 对应一个 Log 对象，在物理磁盘上则对应一个目录。如：创建一个双分区的主题 test，那么，Kafka 会在磁盘上创建两个子目录：test-0 和 test-1；而在服务器端，这就对应两个 Log 对象。

#### Log Segment

Kafka将每个Partition切割成若干个片段，即日志段。默认每个Segment大小不超过1G,且只包含7天的数据。如果Segment的消息量达到1G,那么该Segment会关闭，同时打开一个新的Segment进行写入。

Segment文件命名规则：Partition全局的第一个segment从0开始，后续每个segment文件名为当前 segment 的第一条消息的 offset 。
...
Kafka允许消费者从任意有效的偏移量位置开始读取消息。Kafka为每个Partition都维护了一个索引（即.index文件），该索引将偏移量映射到片段文件以及偏移量在文件里的位置。
...
1. 节省空间：
   ...
   - 如果每条消息都要存一条索引，索引文件会非常大；
   - 稀疏索引只存部分偏移量和磁盘位置，大幅减少索引文件大小；
   - Kafka中一个Partition可能有几百万条消息，稠密索引会占用大量磁盘和内存，而稀疏索引很小，可以常驻内存。
...
2. 加快查找速度（减少I/O）：
   ...
   这样既不用全表扫描，也不需要维护庞大的稠密索引，性能和空间达到平衡。

3.  写入性能更高：

   - Kafka 强调顺序写，高吞吐；
   - 如果是稠密索引，每写一条消息就要更新索引，带来额外开销；
   - 稀疏索引只需要 定期写索引，减少了写放大，保证了高吞吐。

4. 易于段文件管理：

   - Kafka的日志是分段（Segment）存储的；
   - 每个Segment都有对应的稀疏索引文件；
   - 因为索引稀疏，所以文件小，段文件轮转、删除和加载都更高效。

5. 内存友好

   - Kafka在Broker端会把索引mmap到内存（零拷贝）
   - 稀疏索引文件很小，适合 mmap 整个文件到内存，查询时几乎是 O(1) 的。
   - 如果是稠密索引，mmap 占用内存过大，不现实。
...
1. Consumer发送请求到Broker
2. Broker在Partition的Segment文件中找到对应offset（用稀疏索引定位）
3. 数据在Page Cache（或mmap中）
4. Broker调用Sendfile()直接把数据发送给Consumer
